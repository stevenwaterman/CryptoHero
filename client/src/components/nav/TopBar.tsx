...
export default class TopBar extends React.Component<TopBarProps> {
    render(): React.ReactElement<any, string | React.JSXElementConstructor<any>> | string | number | {} | React.ReactNodeArray | React.ReactPortal | boolean | null | undefined {
        return (
            <Navbar bg="primary" variant="dark" className="mb-3 pt-0">
                <Navbar.Brand className="mx-auto px-0">
                ...
                <Form className="form-inline">
                    <Form.Control as="select" value={this.props.selectedId} onChange={event => {
                        ...
                        {this.props.accounts.map(id => <option key={id} value={id}>{id.split("-")[0]}</option>)}
                    </Form.Control>
