...
describe("constructor", () => {
  ...
  test("constructor should return a InstrumentBroker", () => {
    expect(new InstrumentBroker(INSTRUMENTS.GBPBTC)).toBeInstanceOf(
      InstrumentBroker
    );
  });
...
describe("adding orders", () => {
  ...
  test("can't add a buy order that they can't afford", () => {
    acc1.adjustAssets(ASSETS.BTC, new Big("-900"));
    expect(() => {
      iBroker.place(buyOrder);
    }).toThrow();
  });
  ...
  test("can't add a sell order that they can't afford", () => {
    acc1.adjustAssets(ASSETS.GBP, new Big("-950"));
    expect(() => {
      iBroker.place(sellOrder);
    }).toThrow();
  });
...
describe("matching orders", () => {
  ...
  test("Invalid Direction", () => {
    ...
    const order = new Order(1, direction, new Big("1"), new Big("1"));
    expect(() => {
      iBroker.place(order);
    }).toThrow();
  });
...
describe("position updated after order", () => {
  test("BUY 1x1", () => {
    placeBuy(acc1, new Big("1"), new Big("1"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("1000"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("999"));
  });

  test("SELL 1x1", () => {
    placeSell(acc1, new Big("1"), new Big("1"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("999"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1000"));
  });

  test("BUY 2.5x2.5", () => {
    placeBuy(acc1, new Big("2.5"), new Big("2.5"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("1000"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("993.75"));
  });

  test("SELL 2.5x2.5", () => {
    placeSell(acc1, new Big("2.5"), new Big("2.5"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("997.5"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1000"));
  });
});
...
describe("position updated after trade", () => {
  test("1x1", () => {
    placeBuy(acc1, new Big("1"), new Big("1"));
    placeSell(acc2, new Big("1"), new Big("1"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("1001"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("999"));
  ...
  test("2x2", () => {
    ...
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("996"));
    expect(acc2.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("998"));
    expect(acc2.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1004"));
  });

  test("2.5x2", () => {
    placeBuy(acc1, new Big("2.5"), new Big("2"));
    placeSell(acc2, new Big("2.5"), new Big("2"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("1002.5"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("995"));
    expect(acc2.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("997.5"));
    expect(acc2.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1005"));
  });
  ...
  test("2.5x2.5", () => {
    ...
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("993.75"));
    expect(acc2.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("997.5"));
    expect(acc2.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1006.25"));
  });
  ...
  test("2x-1.5", () => {
    ...
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("1002"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1003"));
    expect(acc2.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("998"));
    expect(acc2.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("997"));
  });
  ...
  test("different prices", () => {
    ...
    placeSell(acc2, new Big("1"), new Big("1"));
    expect(acc1.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("1001"));
    expect(acc1.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("998"));
    expect(acc2.getAvailableAssets(ASSETS.GBP)).toEqual(new Big("999"));
    expect(acc2.getAvailableAssets(ASSETS.BTC)).toEqual(new Big("1002"));
  });
});
